...
class RollbackTimeline:
    def __init__(self,db:SQLiteDB):
        self.db=db
        self.db.execute("""CREATE TABLE IF NOT EXISTS v5_rollback_timeline(
    ...
    def _git(root:Path,*args:str)->str:
        p=subprocess.run(["git","-C",str(root),*args],capture_output=True,text=True,shell=False,timeout=30)
    ...
    def capture(self,project_root:str|Path,label:str="checkpoint",metadata:dict[str,Any]|None=None)->dict[str,Any]:
        root=Path(project_root).resolve(); head=self._git(root,"rev-parse","HEAD"); status=self._git(root,"status","--porcelain")
        cid=new_id("rb_")
        self.db.execute("INSERT INTO v5_rollback_timeline VALUES(?,?,?,?,?,?,?)",(cid,str(root),head,status,label[:160],utc_now(),json.dumps(metadata or {},ensure_ascii=False)))
        return self.get(cid)

    def get(self,cid:str):
        row=self.db.execute("SELECT * FROM v5_rollback_timeline WHERE checkpoint_id=?",(cid,),fetch="one")
        if row: row["metadata"]=json.loads(row.pop("metadata_json"))
        return row

    def list(self,project_root:str|Path|None=None,limit:int=50):
        if project_root:
            rows=self.db.execute("SELECT checkpoint_id FROM v5_rollback_timeline WHERE project_root=? ORDER BY created_at DESC LIMIT ?",(str(Path(project_root).resolve()),limit),fetch="all") or []
    ...
    def restore_tracked(self,cid:str,*,approved:bool=False)->dict[str,Any]:
        ...
        self._git(root,"restore","--source",row["git_head"],"--staged","--worktree","--",".")
        return {"restored":True,"checkpoint_id":cid,"head":current,"untracked_preserved":True}
