...
def planner_system_nudge(
    ...
    return (
        ...
        "the v3.5 universal loop: device_computer_observe -> "
        "device_computer_action -> device_computer_verify. In auto mode, UIA is "
        "preferred and OmniParser is used only when configured and UIA has no "
        "usable controls; use vision='always' for custom-rendered visual UI. "
        "Only act on element_id values returned by the current observation_id. "
        "Never invent raw x/y coordinates. Re-observe whenever the screen or "
        "foreground changes. A successful computer action verifies only input "
        "delivery, not the user's goal. Call device_computer_verify and treat "
        "PASS as success; FAIL means continue/recover; INCONCLUSIVE is not success. "
