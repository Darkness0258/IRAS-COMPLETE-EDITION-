...
def open_chat_and_verify(
    ...
    *,
    ...
    This is a bounded deterministic fast path for a very common user goal. It
    performs no composer typing and never presses Enter. Each state-changing UI
    input is bound to one fresh observation, and the observation is consumed
    before the next action. No action is replayed automatically.
    """
    ...
    # If UIA was insufficient for navigation, obtain one visual grounding pass.
    ...
    if contact_target is None:
        ...
        # Search typing is local UI state only. Do not press Enter; wait for
        # search results and then re-observe before any next state-changing input.
    ...
    clicked = computer.action(
        ...
        verify=False,
        # The controller has already excluded the search field/right-pane header
        # and required a unique best contact result. Model callers cannot set this.
    ...
    if header is None:
        # One bounded read-only retry covers slow WebView paint without replaying
        # the click. It is explicitly not an automatic state-changing recovery.
        time.sleep(0.35)
        verification = _fresh_visual_observation(computer)
        observations += 1
        header = _observation_header(verification, contact=contact)
