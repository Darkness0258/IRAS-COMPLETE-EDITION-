...
def make_tools(v5):
    ...
    # RC11 calm deliberation / repair planning -----------------------------------------------
    add(Tool("v5_deliberation_status", "Show RC11 deliberate-calm reasoning and repair policy.", _schema(), lambda: v5.deliberation.status(), PermissionLevel.READ))
    add(Tool("v5_deliberate_repair", "Classify a failure and rank materially different, reversible repair routes before acting. This tool plans only; normal permission gates still govern execution.", _schema({"problem": {"type": "string", "minLength": 1, "maxLength": 12000}, "evidence": _obj(), "attempts": {"type": "array", "items": _obj(), "maxItems": 100}, "goal": _str(4000), "risk": {"type": "string", "enum": ["low", "normal", "high"]}}, ["problem"]), lambda problem, evidence=None, attempts=None, goal="", risk="low": v5.deliberation.plan(problem, evidence=evidence or {}, attempts=attempts or [], goal=goal, risk=risk), PermissionLevel.READ))

    # RC12 goal continuity -------------------------------------------------------------------
    add(Tool("v5_continuity_status", "Inspect the restart-safe RC12 goal continuity ledger. It grants no external authority.", _schema(), lambda: v5.continuity.status(), PermissionLevel.READ))
    add(Tool("v5_continuity_list", "List persistent goal-continuity runs.", _schema({"status": _str(32), "limit": {"type": "integer", "minimum": 1, "maximum": 1000}}), lambda status="", limit=100: v5.continuity.list(status=status, limit=limit), PermissionLevel.READ))
    add(Tool("v5_continuity_begin", "Begin or resume-by-run-key a persistent goal continuity ledger. This records intent/context only and does not execute tools.", _schema({"goal": {"type": "string", "minLength": 1, "maxLength": 12000}, "run_key": _str(240), "metadata": _obj(), "confidence": {"type": "number", "minimum": 0, "maximum": 1}}, ["goal"]), lambda goal, run_key="", metadata=None, confidence=.5: v5.continuity.begin(goal, run_key=run_key, metadata=metadata or {}, confidence=confidence), PermissionLevel.SAFE_ACTION))
    add(Tool("v5_continuity_get", "Read one continuity run and its recent evidence/action ledger.", _schema({"run_id": _str(100), "event_limit": {"type": "integer", "minimum": 1, "maximum": 500}}, ["run_id"]), lambda run_id, event_limit=50: v5.continuity.get(run_id, event_limit=event_limit), PermissionLevel.READ))
    add(Tool("v5_continuity_record", "Record bounded observation/action/verification/failure/repair/checkpoint/blocker/completion evidence. Recording never replays or executes the described action.", _schema({"run_id": _str(100), "kind": {"type": "string", "enum": ["observe", "action", "verification", "failure", "repair", "checkpoint", "blocker", "completion"]}, "summary": {"type": "string", "minLength": 1, "maxLength": 12000}, "payload": _obj(), "confidence": {"type": "number", "minimum": 0, "maximum": 1}, "reversible": {"type": "boolean"}, "world_fingerprint": _str(240)}, ["run_id", "kind", "summary"]), lambda run_id, kind, summary, payload=None, confidence=.7, reversible=True, world_fingerprint="": v5.continuity.record(run_id, kind, summary, payload=payload or {}, confidence=confidence, reversible=reversible, world_fingerprint=world_fingerprint), PermissionLevel.SAFE_ACTION))
    add(Tool("v5_continuity_assess", "Assess a continuity run and choose continue/verify/repair/reobserve/stop from persistent evidence freshness. Advisory only; it cannot grant permission or execute an action.", _schema({"run_id": _str(100), "world_fingerprint": _str(240), "max_observation_age_seconds": {"type": "integer", "minimum": 1, "maximum": 86400}}, ["run_id"]), lambda run_id, world_fingerprint="", max_observation_age_seconds=120: v5.continuity.assess(run_id, world_fingerprint=world_fingerprint, max_observation_age_seconds=max_observation_age_seconds), PermissionLevel.READ))
    add(Tool("v5_continuity_cancel", "Cancel an internal continuity run without replaying any side effect.", _schema({"run_id": _str(100), "reason": _str(1000)}, ["run_id"]), lambda run_id, reason="cancelled": v5.continuity.cancel(run_id, reason=reason), PermissionLevel.SAFE_ACTION))
    # Voice + visual fusion ------------------------------------------------------------------
