...
RC3 includes a local-owner **Master Control** mode for CRITICAL registered-tool execution across local IRAS, the Windows cloud client, web/PWA and Android. The PC must be locally armed first (`iras --master-enable 30`); web/mobile cannot enable elevation by themselves. While Master is active, IRAS uses an **Emergency Adaptive** execution profile: ordinary small tool/task budgets are replaced by high-capacity owner-configurable budgets and an owner-intent continuation rule that keeps working/replanning until the goal is verified complete or a concrete blocker exists. A runaway watchdog still prevents a broken provider/tool loop from running forever. Emergency stop, Remote authentication, filesystem roots, audit logging, Windows/UAC and applicable safety/security restrictions remain enforced. See `docs/MASTER_CONTROL_RC3.md` and `docs/EMERGENCY_MASTER_EXECUTION.md`.
