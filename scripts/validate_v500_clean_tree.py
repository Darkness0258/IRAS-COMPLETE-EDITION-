...
REQUIRED=[
 'README.md','ARCHITECTURE.md','RC4-FEATURE-MATRIX.md','RELEASE-MANIFEST.json',
 'docs/V5_0_RC4_NOTES.md','tests/test_v500_capability_layer.py','tests/test_v500_rc3_complete_operating_layer.py',
 'tests/test_v500_rc3_all_features.py','tests/test_v500_rc3_managed_vision.py','tests/test_v500_rc4_research_installer.py',
 'scripts/validate_v500_integrated.py','scripts/validate_v500_clean_tree.py','scripts/package_v500_rc4.py','run-v500-validation.ps1',
...
def main():
    ...
    ignored_roots={'.venv','.git','.tox','.nox','node_modules'}
    for p in ROOT.rglob('*'):
        relative=p.relative_to(ROOT)
        if any(part in ignored_roots for part in relative.parts):
            continue
        rel=str(relative)
        if {'__pycache__','.pytest_cache','.mypy_cache','.ruff_cache'} & set(relative.parts) or p.suffix in {'.pyc','.pyo'}: cache.append(rel)
        if any(part in {'build','dist'} or part.endswith('.egg-info') for part in relative.parts): debris.append(rel)
    ...
    print('=== IRAS v5.0 RC4 CLEAN TREE VALIDATION ===')
