...
def main() -> None:
    print("=== IRAS v4.1 RC1 INTEGRATED VALIDATION ===")
    assert __version__ == "4.1.0-rc1"
    ...
    def runner(prompt, _context):
        time.sleep(0.08)
        return {"result": prompt.upper(), "metrics": {"total_ms": 80}}

    manager = MultitaskManager(runner, max_workers=3, max_tasks_per_run=8)
    try:
    ...
    assert parse_parallel_command("/parallel one || two || three") == [
        "one", "two", "three"
    ]

    sample = "v4.1-secret-roundtrip"
