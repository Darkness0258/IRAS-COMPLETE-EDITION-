...
Write-Host "=== IRAS v4.1 RC1 MULTITASKING VALIDATION ==="
Write-Host ""
Invoke-IrasCleanup
Write-Host "IRAS validation debris cleanup: PASS"

try {
    python .\scripts\validate_v410_clean_tree.py
    if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }

    Write-Host ""
    python .\scripts\validate_v410_integrated.py
    if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
    ...
    Write-Host ""
    Write-Host "IRAS v4.1 RC1 VALIDATION RESULT: PASS"
}
finally {
    Invoke-IrasCleanup
}
