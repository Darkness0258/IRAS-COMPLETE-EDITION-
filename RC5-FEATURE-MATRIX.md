# IRAS v5.0.0 RC5 — Complete Feature Matrix

RC5 declares **36 operating-layer feature families**. A feature is counted only when its implementation is present, reachable through an IRAS control surface, protected by the existing authorization model where state can change, and covered by local regression/contract tests.
...
Remote protocol remains **1**. RC5 does not replace the v4.4 ToolRegistry, authenticated Remote sessions, laptop-local Remote policy, emergency stop, device-bridge executor/agent, configured filesystem roots, or Windows/UAC boundaries.


## RC5 software lifecycle extension

- Read available upgrades: `device_software_upgrades` / `/install updates [exact-id]`.
- Targeted update: `/install update <software-or-id>` resolves exactly one installed package before CRITICAL execution.
- Explicit bulk update: `/install update all`; never inferred from a vague request.
- Uninstall: `/install uninstall <software-or-id>` resolves exactly one installed package and verifies absence after execution.
- Install/update/uninstall retain Remote Protocol 1, ToolRegistry, authenticated Remote session, local `allow_shell`, emergency stop, audit, Windows/UAC, and shell-free fixed argv execution.
