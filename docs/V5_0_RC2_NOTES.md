# IRAS v5.0.0 RC2 — Editable-Install Validation Hotfix

RC2 fixes the release-validation sequence after `pip install -e ".[all,cloud]"`. Editable installs generate `*.egg-info` metadata inside the source tree; the validator now removes reproducible packaging debris (`*.egg-info`, `build`, `dist`) before clean-tree checks and again after the suite. Runtime behavior and remote protocol remain unchanged.

## Base v5 autonomous operating layer
